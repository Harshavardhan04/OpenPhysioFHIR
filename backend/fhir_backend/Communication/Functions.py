...
def getNotes(id: str, path: str, filename: str = None) -> list:
    ...
    if filename is None:  # return all notes for patient
        # Get all file paths
        file_paths = [os.path.join(path, id, name) for name in os.listdir(os.path.join(path, id))]
...
def getLatestConsultation(patient_id: int, fhir_path: str, notes_path: str, desired_path: str) -> dict:
    """
    ...
    latest_note = all_notes[-1]['content'][0]['attachment']['data']  # Assuming the last note in the list is the latest
    print("latest note", latest_note)


    
    # Get the desired SNOMED codes
    
    # Get the latest observation for each SNOMED code using the index of the latest note
    data = getImprovement(str(patient_id), fhir_path, getDesired(str(patient_id), os.path.join(fhir_path, "Desired")))
    latest_observations = {key: values[-1] for key, values in data.items() if values}
    print("latest obs", latest_observations)
    # latest_observations, _ = getInterval(patient_id, latest_note_index + 1, latest_note_index + 1, fhir_path, notes_path)
    # print("latest obs", latest_observations)
    
    return latest_observations, latest_note
# You would call the function like this:
# latest_consultation_details = getLatestConsultation(patient_id, fhir_path, notes_path, desired_path)
